...
library(ggplot2)
library(MASS)
df  <- read.table('8552.txt', header = T)
...
AFD_class <- function(df){
  ...
  gr_ind_1 <- gr_ind_1   +  xlim(c(-5,4)) + ylim(c(-3,3))  # uniformit̩s des ̩chelles
  print(gr_ind_1)
  ...
  gr_ind_2 <- ggplot() +  geom_point(data = G_fac  ,aes( x = Axe_1, y = Axe_2, colour = Group.1, shape =  Group.1), size = 3)  +  # trace les centres de gravit̩
  ...
  gr_ind_3 <- ggplot() + geom_point(data = Scores, aes(x = Axe_1, y = Axe_2, colour = class), size = 2)    +
    ...
    geom_point(data = G_fac,  aes(x = Axe_1, y = Axe_2, colour = Group.1, shape =  Group.1), size = 3)  +  # trace les centres de gravit̩
